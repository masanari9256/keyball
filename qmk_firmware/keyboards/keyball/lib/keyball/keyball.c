...
#include "timer.h"  // タイマー関連のヘッダファイルをインクルード

uint8_t cat_state = 0;  // 現在の猫の状態を保持
static uint16_t last_update_time = 0;  // 最後に更新した時間を保持

void keyball_oled_render_cat(void) {
#ifdef OLED_ENABLE
    // 0.2秒（200ms）経過した場合に画像を更新
    if (timer_elapsed(last_update_time) > 100) {
        // cat_state の値に応じて画像を表示
        switch (cat_state) {
            case 0:
        ...
        // cat_stateをインクリメントして、範囲を超えたらリセット
        cat_state = (cat_state + 1) % 5;  // 0から4までループ
